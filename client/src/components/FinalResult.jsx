...
function FinalResult({result}) {
  ...
  return (
    ...
    <div className='flex flex-col md:flex-row md:items-center  md:justify-between gap-4'>
      ...
      <div className='flex gap-3'>
        ...
        :"bg-green-100 hover:bg-green-200" }`}>
          {quickRevision ? "Exist Revison Mode": "Quick Revision (5 min)"}
        </button>
   ...
   {!quickRevision &&
    <section className='bg-white border-gray-200 rounded-xl p-6'>
    ...
    <section className='rounded-xl bg-gradient-to-r from-green-100 to-green-50 border border-green-200 p-6'>
      ...
      <ul className='list-disc ml-6 space-y-1 text-gray-800'>
         {result.data.revisonPoints.map((p,i)=>(
          <li key={i}>{p}</li>
         ))}
      </ul>
    </section>
   ...
   {result.data.diagrams?.data &&
    <section>
      <SectionHeader icon="📊" title="Diagram" color="cyan" />
      <MermaidSetup  diagram={result.data.diagrams?.data} />
      <p className='mt-3 text-xs text-gray-500 italic'>
        ℹ️ If you need this diagram for future refrence or revision, you can save it by taking a screenshot
      </p>
     ...
     <section>
        ...
        <p className='mt-3 text-xs text-gray-500 italic'>
          ℹ️If you need this Chart for future refrence or revision,
    ...
    <section>
      ...
      <ul className='list-disc ml-6 text-gray-700'>
         <li>{result.data.questions.diagrams}</li>
      </ul>
    </section>
