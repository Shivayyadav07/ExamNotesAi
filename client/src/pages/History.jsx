...
function History() {
  ...
  const openNotes = async (noteId) => {
    ...
    try {
      const res = await axios(serverUrl + `/api/notes/${noteId}`, {
        withCredentials: true,
      });
       
      setSelectedNotes(res.data.data.content);
    } catch (error) {
      console.log(error);
    } finally {
      setLoading(false);
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-100 to-gray-200 px-6 py-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
                ...
                <ul className="space-y-3">
                  {topics.map((t, i) => (
                    <li
                      ...
                      className="cursor-pointer rounded-xl bg-white/5 border border-white/10 hover:bg-white/10"
                    >
                      <p className="text-sm font-semibold m-3 text-white">
                        {t.topic}
                      </p>
                      <div className="flex flex-wrap gap-2 mt-2 text-xs">
                        {t.examType && (
                          <span className="px-2 py-0.5 rounded-full bg-purple-500/20 text-indigo-300">
                            {t.examType}
                          </span>
                        )}
                      </div>
        ...
        >
          ...
          {!loading && selectedNotes && (
            <FinalResult result={{ data: selectedNotes }} />
          )}
        </motion.div>
      </div>
    </div>
